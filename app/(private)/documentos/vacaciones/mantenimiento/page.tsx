...
// marcadas explícitamente es_prueba=true desde el detalle (ver VacationTestToggle), y nunca una
// FIRMADO (esa siempre queda fuera de este listado, aunque alguien la hubiera marcado antes).
export default async function VacationTestMaintenancePage() {
  ...
  const [{ data }, { data: pending }] = await Promise.all([
    db.from("papeletas_vacaciones").select("id,colaborador_nombre,colaborador_codigo,estado,created_at")
      .eq("es_prueba", true).neq("estado", "FIRMADO").order("created_at", { ascending: false }).limit(200),
  ...
  return <section className="mx-auto max-w-3xl">
    ...
    <header className="page-header">
      ...
      <p className="page-description">Elimina físicamente papeletas de prueba marcadas explícitamente por un administrador, para no consumir espacio de Storage. Nunca elimina documentos reales ni firmados.</p>
    </header>
