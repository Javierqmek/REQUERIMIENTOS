  ...
  try{
    if(action==="colocaciones"){
      ...
      try{
        ...
        const {data:placements,error:placementsError}=await db.from("documento_firmas").select("tipo,pagina,x,y,ancho,alto,asset_path").eq("documento_id",id);if(placementsError)throw placementsError;
