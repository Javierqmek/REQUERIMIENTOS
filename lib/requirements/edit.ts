...
export type EditLine = { prenda_id: string; detalle_id?: string; nombre: string; cantidad: number; precio: number; codigo: string };
...
export function initialEditLines(row: EditableRequirement): EditLine[] {
  return row.detalle_requerimiento.filter(d => d.activo).map(d => ({
    ...
    cantidad: d.prendas.cantidad, precio: Number(d.precio_unitario), codigo: d.codigo_almacen,
  }));
}
export function newEditLine(p: Prenda): EditLine {
  return { prenda_id: p.id, nombre: p.nombre_prenda, cantidad: p.cantidad, precio: Number(p.precio), codigo: p.codigo_almacen };
}
