...
import { FileSignature, FileText, PlaneTakeoff } from "lucide-react";
import type { PapeletaRow } from "@/lib/vacations/types";
import { PapeletaEstadoBadge } from "./papeleta-estado-badge";

// Bandeja compacta del gerente: SOLO lo que el requerimiento pide ver en la tarjeta (colaborador,
// estado, fecha de registro, coordinador, cliente, unidad). Todo lo demás (fechas físicas/venta,
// días, reemplazo, provincia) vive en "Ver detalle", nunca en esta tarjeta.
export function VacationManagerInbox({ rows }: { rows: PapeletaRow[] }) {
  if (!rows.length) return <div className="card px-5 py-8 text-center">
    <FileText className="mx-auto text-[#8794A8]" size={28} />
    <p className="mt-3 text-sm font-medium text-[#45556D]">No hay papeletas de vacaciones por revisar.</p>
  </div>;

  return <div className="grid min-w-0 gap-2.5">
    {rows.map(row => {
      const puedeFirmar = row.estado === "REGISTRADO";
      return <div key={row.id} className={`card grid min-w-0 gap-3 p-4 sm:flex sm:items-center sm:justify-between ${row.estado === "OBSERVADO" ? "border-amber-300" : ""}`}>
        <div className="flex min-w-0 items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 truncate text-xs text-[#607089]">
              Registrado: {new Intl.DateTimeFormat("es-PE", { dateStyle: "medium" }).format(new Date(row.created_at))}
              {row.profiles?.nombre ? ` · Coordinador: ${row.profiles.nombre}` : ""}
            </p>
