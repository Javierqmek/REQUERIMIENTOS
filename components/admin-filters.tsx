  ...
  return <form onSubmit={e => { e.preventDefault(); onApply(); }} className="border-t border-[var(--border)] px-4 pb-4 pt-3 sm:px-5">
    <fieldset disabled={disabled} className="min-w-0">
      ...
      <div className="grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="min-w-0"><label className="label" htmlFor="admin-genero">Género de prenda</label><select className="input" id="admin-genero" value={value.genero} onChange={e => change("genero",e.target.value)}><option value="">Todos los géneros</option><option value="HOMBRE">Hombre</option><option value="MUJER">Mujer</option><option value="AMBOS">AMBOS / Unisex</option></select></div>
        <div className="grid grid-cols-2 gap-2"><div><label className="label" htmlFor="admin-unidades-min">Unidades mín.</label><input className="input" id="admin-unidades-min" inputMode="numeric" value={value.unidadesMin} onChange={e=>change("unidadesMin",e.target.value)} placeholder="0"/></div><div><label className="label" htmlFor="admin-unidades-max">Unidades máx.</label><input className="input" id="admin-unidades-max" inputMode="numeric" value={value.unidadesMax} onChange={e=>change("unidadesMax",e.target.value)} placeholder="Sin límite"/></div></div>
