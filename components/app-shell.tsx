...
export function AppShell({profile,children}:{profile:Profile;children:React.ReactNode}){
  ...
  return <div className="min-h-screen pb-20 sm:pb-0">
    <header className="sticky top-0 z-30 border-b border-[#DCE3EC] bg-white/95 backdrop-blur"><div className="mx-auto flex h-16 max-w-7xl items-center gap-7 px-4 sm:px-6">
      ...
      <nav className="hidden h-full items-center gap-1 sm:flex" aria-label="Navegación principal">{links.map(({href,label})=><Link key={href} href={href} className={`flex h-full items-center border-b-2 px-3 text-sm font-medium ${active(href)?"border-[#2563EB] text-[#174EA6]":"border-transparent text-[#607089] hover:text-[#172033]"}`}>{label}</Link>)}</nav>
    ...
    <nav className="fixed inset-x-0 bottom-0 z-20 border-t border-[#DCE3EC] bg-white sm:hidden" aria-label="Navegación móvil"><div className="grid" style={{gridTemplateColumns:`repeat(${links.length},minmax(0,1fr))`}}>{links.map(({href,label,icon:Icon})=><Link key={href} href={href} className={`flex min-h-16 flex-col items-center justify-center gap-1 px-1 text-[10px] font-medium ${active(href)?"text-[#174EA6]":"text-[#607089]"}`}><Icon size={19}/><span className="max-w-full truncate">{label}</span></Link>)}</div></nav>
