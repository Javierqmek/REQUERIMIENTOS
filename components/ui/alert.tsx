...
export function Alert({kind="info",children}:{kind?:Kind;children:React.ReactNode}){const Icon=icons[kind];return <div role={kind==="error"?"alert":"status"} className={`flex items-start gap-3 rounded-xl border p-4 text-sm font-medium ${styles[kind]}`}><Icon className="mt-0.5 shrink-0" size={19}/><div>{children}</div></div>}
