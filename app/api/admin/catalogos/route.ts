...
export async function POST(request:Request){try{if((await getCurrentProfile())?.role!=="admin")return response({error:"No autorizado"},403);const input=catalogCreateSchema.parse(await readJsonBody(request,16384));const {data,error}=await (await createClient()).rpc("admin_guardar_catalogo",{p_catalogo:input.catalogo,p_id:null,p_valores:input.valores});if(error)throw error;return response(data,201);}catch(error){return failure(error,"No se pudo crear el registro.");}}
export async function PUT(request:Request){try{if((await getCurrentProfile())?.role!=="admin")return response({error:"No autorizado"},403);const input=catalogUpdateSchema.parse(await readJsonBody(request,16384));const {data,error}=await (await createClient()).rpc("admin_guardar_catalogo",{p_catalogo:input.catalogo,p_id:input.id,p_valores:input.valores});if(error)throw error;return response(data);}catch(error){return failure(error,"No se pudo guardar el registro.");}}
...
export async function DELETE(request:Request){try{if((await getCurrentProfile())?.role!=="admin")return response({error:"No autorizado"},403);const input=catalogDeleteSchema.parse(await readJsonBody(request,4096));const {data,error}=await (await createClient()).rpc("admin_eliminar_catalogo",{p_catalogo:input.catalogo,p_id:input.id});if(error)throw error;return response(data);}catch(error){return failure(error,"No se pudo eliminar el registro.");}}
