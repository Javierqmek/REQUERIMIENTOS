...
export async function POST(_:Request,{params}:{params:Promise<{id:string}>}){
 ...
 const {data:document,error}=await db.from("documentos").select("usuario_creador_id,firmante_id,estado,archivo_original_path,archivo_coordinador_path").eq("id",id).maybeSingle();
 ...
 try{
  const source=manager?documentBasePath(document):document.archivo_original_path;
  const result=await createSignedPdf(db,source,(rows||[]).map(row=>({...row,x:Number(row.x),y:Number(row.y),ancho:Number(row.ancho),alto:Number(row.alto)}) as Placement));
  return new NextResponse(new Blob([asArrayBuffer(result.bytes)],{type:"application/pdf"}),{headers:{"Content-Type":"application/pdf","Cache-Control":"private, no-store","X-Content-Type-Options":"nosniff","X-Preview-SHA256":result.hash}});
 }catch(error){
